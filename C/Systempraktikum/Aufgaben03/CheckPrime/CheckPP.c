...
int main(int argc, char *argv){
    ...
    if (guessed % 2 == 0) {result = false;} 
        ...
        int pid = fork();
        if (pid == 0){  // KIND
            switch (checkForPrimeIt(guessed, 2, guessed/7)){
          ...
          waitpid(pid, NULL, 0);
          read(fd[0], &test, sizeof(test));
            switch (test) {
              case 1  : i = checkForPrimeIt(guessed, guessed/7, guessed/6));
                        write(fd[1], &i, sizeof(i); exit(0); break;
